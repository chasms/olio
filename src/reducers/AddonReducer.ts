import cuid from 'cuid';
import type { AddonItem, AddonActions } from '../types/actions';
import { ActionTypes } from '../types/actions';

export type AddonState = AddonItem[];

export default function Addon(state: AddonState = [], action: AddonActions): AddonState {
  switch (action.type) {
    case ActionTypes.ADD_ADDON:
      return [
        ...state,
        {
          x: 50,
          y: 100,
          h: action.payload.initial_height,
          w: action.payload.initial_width,
          url: action.payload.url,
          category: action.payload.category,
          fontFamily: action.payload.fontFamily,
          id: cuid(),
        },
      ];
    case ActionTypes.REMOVE_ADDON:
      return state.filter((a) => a.id !== action.payload.id);
    case ActionTypes.STORE_LOCATION: {
      const found = state.find((addon) => addon.id === action.payload.id);
      if (!found) return state;
      const rest = state.filter((addon) => addon.id !== action.payload.id);
      const newLocation: AddonItem = {
        id: found.id,
        url: found.url,
        y: action.payload.coordinates.top,
        x: action.payload.coordinates.left,
        h:
          found.category === 'text'
            ? action.payload.coordinates.height - 4
            : action.payload.coordinates.height,
        w:
          found.category === 'text'
            ? action.payload.coordinates.width - 4
            : action.payload.coordinates.width,
        category: found.category,
        fontFamily: found.fontFamily,
      };
      if (newLocation.category === 'text') newLocation.value = action.payload.value;
      return [...rest, newLocation];
    }
    case ActionTypes.DELETE_ADDONS:
      return [];
    case ActionTypes.RESTORE_CREATION:
      return action.payload.composition;
    default:
      return state;
  }
}
